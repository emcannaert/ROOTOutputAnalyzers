...
class clusteringAlgorithmSingleLepton : public edm::EDAnalyzer 
{
public:
   explicit clusteringAlgorithmSingleLepton(const edm::ParameterSet&);
private:
   ...
   int nGenLeptons = 0, nGenNeutrinos =0;
   double SJ_E[2],SJ_Px[2],SJ_Py[2],SJ_Pz[2];
   double neutrino_pt[100],lepton_pt[100];
   double thrustLeptonAngle[100];
   double posSJP =0;
   double negSJP = 0;
   double daughter_mass_comb[100];
   double leptonMET_angle_lab[100], leptonMET_angle_MPP[100];
   double resonance_mass_comb;
   int nLeptons = 0;
   int nZtZt = 0, nhtht = 0, nWbWb = 0, nZtWb = 0, nZtht = 0, nhtWb = 0;
};

clusteringAlgorithmSingleLepton::clusteringAlgorithmSingleLepton(const edm::ParameterSet& iConfig)
{
   ...
   tree->Branch("nGenNeutrinos", &nGenNeutrinos, "nGenNeutrinos/I");
   tree->Branch("lepton_pt", lepton_pt, "lepton_pt[nGenLeptons]/D");
   tree->Branch("neutrino_pt", neutrino_pt, "neutrino_pt[nGenLeptons]/D");
   tree->Branch("thrustLeptonAngle", thrustLeptonAngle, "thrustLeptonAngle[nLeptons]/D");

   tree->Branch("leptonMET_angle_MPP", leptonMET_angle_MPP, "leptonMET_angle_MPP[nLeptons]/D");
   tree->Branch("leptonMET_angle_lab", leptonMET_angle_lab, "leptonMET_angle_lab[nLeptons]/D");



   //tree->Branch("lnu_angle_COM", lnu_angle_COM, "lnu_angle_COM[nGenLeptons]/D");
   //tree->Branch("lnu_angle_MPP", lnu_angle_MPP, "lnu_angle_MPP[nGenLeptons]/D");

}


double clusteringAlgorithmSingleLepton::calc_mag(double px,double py, double pz)
{
   return sqrt(pow(px,2)+pow(py,2)+pow(pz,2));
}
const reco::Candidate* clusteringAlgorithmSingleLepton::parse_chain(const reco::Candidate* cand)
{  
...
bool clusteringAlgorithmSingleLepton::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF)
{
...
bool clusteringAlgorithmSingleLepton::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF, int nfatjets)
{
...
double clusteringAlgorithmSingleLepton::calcMPP(TLorentzVector superJetTLV ) 
{
...
double clusteringAlgorithmSingleLepton::calc_mass(std::vector<TLorentzVector> candSuperJet)
{
...
bool clusteringAlgorithmSingleLepton::isMatchedtoSJ(std::vector<TLorentzVector> superJetTLVs, TLorentzVector candJet)
{
...
void clusteringAlgorithmSingleLepton::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  edm::Handle<std::vector<pat::Jet> > smallJets;
   ...
   ////////////////////////////MET////////////////////////////////
   ///////////////////////////////////////////////////////////////
   std::vector<reco::LeafCandidate> recoMET;
   std::vector<TLorentzVector> recoMETTLV;
   ...
   for(auto iMET = _totMET->begin(); iMET != _totMET->end(); iMET++)
   {
      ...
      {
         totMET = iMET->pt();
         recoMET.push_back( reco::LeafCandidate(0, reco::Candidate::LorentzVector(iMET->px(), iMET->py(), iMET->pz(), iMET->energy())) );
         recoMETTLV.push_back( TLorentzVector(iMET->px(), iMET->py(), iMET->pz(), iMET->energy())   ) ;
      }
   }
   ...
   ////////////////////////////////////////////////////////////////
   int nLeptonsCut = 0;
   std::vector<reco::LeafCandidate> leptonsLC;
   std::vector<TLorentzVector> leptonsTLV;

   ////// look high pT lepton(s) //////
    int nE =0, nTau = 0, nMuon = 0;
    edm::Handle<std::vector<pat::Muon>> muons;
    iEvent.getByToken(muonToken_, muons);
    for(auto iM = muons->begin(); iM != muons->end();iM++)
    {
      if( (iM->passed(reco::Muon::CutBasedIdMedium)) && (iM->pt() > 8.) && (abs(iM->eta()) < 2.4)  && (iM->passed(reco::Muon::PFIsoMedium))   )
      {
         nMuon++;
         if(iM->pt() > 75.)
         {
            leptonsLC.push_back( reco::LeafCandidate(+1, reco::Candidate::LorentzVector(iM->px(), iM->py(), iM->pz(), iM->energy()))   );
            leptonsTLV.push_back(TLorentzVector(iM->px(), iM->py(), iM->pz(), iM->energy()) );
            nLeptonsCut++;
         }
      }
   ...
   {
      if((iE->electronID("mvaEleID-Fall17-iso-V2-wp90")) && (iE->pt() > 12.) && (abs(iE->eta())<2.5 )    )
      {
         nE++;
         if(iE->pt() > 75.)
         {
            leptonsLC.push_back( reco::LeafCandidate(+1, reco::Candidate::LorentzVector(iE->px(), iE->py(), iE->pz(), iE->energy()))   );
            leptonsTLV.push_back(TLorentzVector(iE->px(), iE->py(), iE->pz(), iE->energy()) );
            nLeptonsCut++;
         }
      }
   }
      ...
      if((iT->pt() > 20.) && (abs(iT->eta()) < 2.3)  )
      {
         if (iT->tauID("byMediumDeepTau2017v2p1VSjet"))   //was medium before
         {
            if (iT->tauID("byLooseDeepTau2017v2p1VSe") && iT->tauID("byMediumDeepTau2017v2p1VSmu"))
            {
               ...
               if(iT->pt() > 75.)
               {
                  leptonsLC.push_back( reco::LeafCandidate(+1, reco::Candidate::LorentzVector(iT->px(), iT->py(), iT->pz(), iT->energy()))   );
                  leptonsTLV.push_back(TLorentzVector(iT->px(), iT->py(), iT->pz(), iT->energy()) );
                  nLeptonsCut++;
               }
            }
         }
   ...
   if(nLeptonsCut>1) return;
   TVector3 leptonLabVector(leptonsTLV[0].Px()/leptonsTLV[0].P(), leptonsTLV[0].Py()/leptonsTLV[0].P(),leptonsTLV[0].Pz()/leptonsTLV[0].P());
   if(nLeptonsCut>0)
   {
      double MET_mag = sqrt(pow(recoMET[0].Px()*leptonLabVector.X(),2)+pow(recoMET[0].Py()*leptonLabVector.Y(),2)+pow(recoMET[0].PZ()*leptonLabVector.Z(),2)  ) ;
      TLorentzVector new_recoMETLV();
      reco::LeafCandidate new_recoMET(+1,reco::Candidate::LorentzVector(   )    );
      recoMET.clear();
      recoMETTLV.clear();
      recoMET.push_back(new_recoMET);
      recoMETTLV.push_back(new_recoMETLV);
   }
   ...
   for (auto iG = genParticles->begin(); iG != genParticles->end(); iG++) 
   {
      if ((abs(iG->pdgId()) == 24) && ((abs(iG->mother()->pdgId()) == chi_pdgid)) ) 
      {
         ...
         for (unsigned int iii=0; iii<W_final->numberOfDaughters(); iii++)
         {
            ...
            else if( (abs(W_daughter->pdgId()) == 11) ||   (abs(W_daughter->pdgId()) == 13) || (abs(W_daughter->pdgId()) == 15) )
            { 
               lepton_pt[nGenLeptons] = W_daughter->pt();
               nGenLeptons++;
            }
            else if( (abs(W_daughter->pdgId()) == 12) ||   (abs(W_daughter->pdgId()) == 14) || (abs(W_daughter->pdgId()) == 16) )
            { 
               neutrino_pt[nGenNeutrinos] = W_daughter->pt();
               nGenNeutrinos++;
            } 
         }
         ...
         for (unsigned int iii=0; iii<h_final->numberOfDaughters(); iii++)
         {
            ...
            else if (abs(h_daughter->pdgId()) ==24)
            {
               ...
               for (unsigned int jjj=0; jjj<higgsW_final->numberOfDaughters(); jjj++)
               {
                  ...
                  else if( (abs(higgsW_daughter->pdgId()) == 11) ||   (abs(higgsW_daughter->pdgId()) == 13) || (abs(higgsW_daughter->pdgId()) == 15) )
                  { 
                     lepton_pt[nGenLeptons] = higgsW_daughter->pt();
                     nGenLeptons++;
                  }
                  else if( (abs(higgsW_daughter->pdgId()) == 12) ||   (abs(higgsW_daughter->pdgId()) == 14) || (abs(higgsW_daughter->pdgId()) == 16) )
                  { 
                     neutrino_pt[nGenNeutrinos] = higgsW_daughter->pt();
                     nGenNeutrinos++;
                  } 
            ...
            else if (abs(h_daughter->pdgId()) ==23)
            {
               ...
               for (unsigned int jjj=0; jjj<higgsZ_final->numberOfDaughters(); jjj++)
               {
                  ...
                  else if( (abs(higgsZ_daughter->pdgId()) == 11) ||   (abs(higgsZ_daughter->pdgId()) == 13) || (abs(higgsZ_daughter->pdgId()) == 15) )
                  { 
                     lepton_pt[nGenLeptons] = higgsZ_daughter->pt();
                     nGenLeptons++;
                  }
                  else if( (abs(higgsZ_daughter->pdgId()) == 12) ||   (abs(higgsZ_daughter->pdgId()) == 14) || (abs(higgsZ_daughter->pdgId()) == 16) )
                  { 
                     neutrino_pt[nGenNeutrinos] = higgsZ_daughter->pt();
                     nGenNeutrinos++;
                  }
               }
            }
      ...
      else if ( (abs(iG->pdgId()) == 23) && ((abs(iG->mother()->pdgId()) == chi_pdgid)) ) 
      {
         ...
         for (unsigned int jjj=0; jjj<Z_final->numberOfDaughters(); jjj++)
         {
            ...
            else if( (abs(Z_daughter->pdgId()) == 11) ||   (abs(Z_daughter->pdgId()) == 13) || (abs(Z_daughter->pdgId()) == 15) )
            { 
               lepton_pt[nGenLeptons] = Z_daughter->pt();
               nGenLeptons++;
            }
            else if( (abs(Z_daughter->pdgId()) == 12) ||   (abs(Z_daughter->pdgId()) == 14) || (abs(Z_daughter->pdgId()) == 16) )
            { 
               neutrino_pt[nGenNeutrinos] = Z_daughter->pt();
               nGenNeutrinos++;
            }       
         }
         nZ++;
...
/////////////Get AK8 jet info in order to get to COM frame and get a vector of all the (good) AK8 jet particles/////////////////////////////////////////////////////////////
   ...
   ///////////////add in leptons/////////////////
   for(auto iLC = leptonsLC.begin();iLC!=leptonsLC.end();iLC++)
   {
      candsUnboosted.push_back(*iLC);
      tot_jet_px+=iLC->px() ; tot_jet_py+=iLC->py() ; tot_jet_pz+=iLC->pz() ; tot_jet_E+=iLC->energy() ;
   }
   ///////////////add in MET/////////////////
   for(auto iMET = recoMET.begin();iMET!=recoMET.end();iMET++)
   {
      candsUnboosted.push_back(*iMET);
      tot_jet_px+=iMET->px() ; tot_jet_py+=iMET->py() ; tot_jet_pz+=iMET->pz() ; tot_jet_E+=iMET->energy() ;
   }
   ...
   nLeptons = 0;
   TVector3 lab_MET(recoMETTLV[0].X(),recoMETTLV[0].Y(),recoMETTLV[0].Z());
   for(auto iMETLV = recoMETTLV.begin(); iMETLV != recoMETTLV.end(); iMETLV++)
   {
      iMETLV->Boost(-totJetBeta.X(),-totJetBeta.Y(),-totJetBeta.Z());
   }
   TVector3 MPP_MET(recoMETTLV[0].X(),recoMETTLV[0].Y(),recoMETTLV[0].Z());


   
   for(auto iTLV = leptonsTLV.begin(); iTLV!=leptonsTLV.end();iTLV++)
   {
      leptonMET_angle_lab[nLeptons] = iTLV->Angle(lab_MET);
      iTLV->Boost(-totJetBeta.X(),-totJetBeta.Y(),-totJetBeta.Z());
      thrustLeptonAngle[nLeptons] = iTLV->Angle(thrust_vector);
      leptonMET_angle_MPP[nLeptons] = iTLV->Angle(MPP_MET);
      nLeptons++;
   } 
...
DEFINE_FWK_MODULE(clusteringAlgorithmSingleLepton);
