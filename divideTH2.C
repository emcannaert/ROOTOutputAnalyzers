...
void divideTH2()
{
  ...
  TH2F *h_HT1000to1500_HT = (TH2F*)f1->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_HT1500to2000_HT = (TH2F*)f2->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_HT2000toInf_HT = (TH2F*)f3->Get("h_MSJ_mass_vs_MdSJ");
  ...
  TH2F *h_WJetsToLNu = (TH2F*)f9->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_WJet_QQ = (TH2F*)f10->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_TTToHadronic = (TH2F*)f11->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_TTTo2l2nu = (TH2F*)f12->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_TTToSemiLeptonic = (TH2F*)f13->Get("h_MSJ_mass_vs_MdSJ");
  ...
  TH2F *h_JetHTA_HT = (TH2F*)f5->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_JetHTB_HT = (TH2F*)f6->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_JetHTC_HT = (TH2F*)f7->Get("h_MSJ_mass_vs_MdSJ");
  TH2F *h_JetHTD_HT = (TH2F*)f8->Get("h_MSJ_mass_vs_MdSJ");
  int totalNDataEvents = h_JetHTA_HT->GetEntries() + h_JetHTB_HT->GetEntries() + h_JetHTC_HT->GetEntries() + h_JetHTD_HT->GetEntries();
  std::cout << "Total number of events is " << totalNDataEvents << std::endl;
  ...
  TH2F *h_JetHT_combined = new TH2F(*h_JetHTA_HT);
  h_JetHT_combined->GetYaxis()->SetTitleOffset(1.35);
  h_JetHT_combined->GetYaxis()->SetLabelSize(0.015);
  h_JetHT_combined->Add(h_JetHTB_HT);
  h_JetHT_combined->Add(h_JetHTC_HT);
  h_JetHT_combined->Add(h_JetHTD_HT);
  h_JetHT_combined->SetTitle("M_{SJ} vs M_{diSJ} for Data (JetHT 2018);M_{diSJ};M_{SJ}");

  h_JetHT_combined->Draw("colz");
  c1->SaveAs("mSJ_vs_mdiSJ_DATA.png"); 
  ...
  h_BR_combined->SetTitle("M_{SJ} vs M_{diSJ} for Combined MC BR;M_{diSJ};M_{SJ}");
  h_BR_combined->GetYaxis()->SetTitleOffset(1.35);
  h_BR_combined->GetYaxis()->SetLabelSize(0.015);

  h_BR_combined->Draw("colz");
  c1->SaveAs("mSJ_vs_mdiSJ_BR.png"); 


  h_JetHT_combined->Divide(h_BR_combined);
  h_JetHT_combined->SetTitle("data/MC ratio of M_{SJ} vs M_{diSJ};M_{diSJ};M_{SJ}");
  h_JetHT_combined->GetYaxis()->SetTitleOffset(1.35);
  h_JetHT_combined->GetYaxis()->SetLabelSize(0.015);

  h_JetHT_combined->Draw("colz");
  c1->SaveAs("mSJ_vs_mdiSJ_ratio.png"); 
...
//There are 3338 total events, with 97 anti-tagged, and of these events there are 82 with second superjet tagged.