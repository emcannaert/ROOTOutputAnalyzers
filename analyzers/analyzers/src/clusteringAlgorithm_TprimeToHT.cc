...
class clusteringAnalyzer_TprimeToHT : public edm::EDAnalyzer 
{
public:
   explicit clusteringAnalyzer_TprimeToHT(const edm::ParameterSet&);
private:
...
clusteringAnalyzer_TprimeToHT::clusteringAnalyzer_TprimeToHT(const edm::ParameterSet& iConfig)
{
...
double clusteringAnalyzer_TprimeToHT::calc_mag(double px,double py, double pz)
{
   return sqrt(pow(px,2)+pow(py,2)+pow(pz,2));
}
const reco::Candidate* clusteringAnalyzer_TprimeToHT::parse_chain(const reco::Candidate* cand)
{  
...
bool clusteringAnalyzer_TprimeToHT::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF)
{
...
bool clusteringAnalyzer_TprimeToHT::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF, int nfatjets)
{
...
double clusteringAnalyzer_TprimeToHT::calcMPP(TLorentzVector superJetTLV ) 
{
...
double clusteringAnalyzer_TprimeToHT::calc_mass(std::vector<TLorentzVector> candSuperJet)
{
...
bool clusteringAnalyzer_TprimeToHT::isMatchedtoSJ(std::vector<TLorentzVector> superJetTLVs, TLorentzVector candJet)
{
...
void clusteringAnalyzer_TprimeToHT::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   for(auto iJet = fatJets->begin(); iJet != fatJets->end(); iJet++)         ////////Over AK8 Jets
   {
      ...
      if((sqrt(pow(iJet->mass(),2)+pow(iJet->pt(),2)) < 100.) || (!(iJet->isPFJet())) || (!isgoodjet(iJet->eta(),iJet->neutralHadronEnergyFraction(), iJet->neutralEmEnergyFraction(),iJet->numberOfDaughters(),iJet->chargedHadronEnergyFraction(),iJet->chargedMultiplicity(),iJet->muonEnergyFraction(),iJet->chargedEmEnergyFraction(),nfatjets)) || (iJet->userFloat("ak8PFJetsPuppiSoftDropMass")< 2.)) continue;
...
//fastjet::PseudoJet(puppiweight*iJ->px(),puppiweight*iJ->py(),puppiweight*iJ->pz(),puppiweight*iJ->energy())
   ...
   for(auto iJet = fatJets->begin(); iJet != fatJets->end(); iJet++)         ////////Over AK8 Jets
   {
      ...
      if((iJet->pt() <300.) || (!(iJet->isPFJet())) || (!isgoodjet(iJet->eta(),iJet->neutralHadronEnergyFraction(), iJet->neutralEmEnergyFraction(),iJet->numberOfDaughters(),iJet->chargedHadronEnergyFraction(),iJet->chargedMultiplicity(),iJet->muonEnergyFraction(),iJet->chargedEmEnergyFraction())) || (iJet->userFloat("ak8PFJetsPuppiSoftDropMass")< 40.)) nfatjet_pre++;
   ...
   //std::cout << "//////// RECO px/py/pz/ptot = " << pxCOM << "/" << pyCOM<<"/" << pzCOM<< "/" << sqrt(pow(pxCOM,2)+pow(pyCOM,2)+pow(pzCOM,2))<< std::endl;
   ...
   for(auto iJet = fatJets->begin(); iJet != fatJets->end(); iJet++)         //get vector of all sorted superjet particles
   {
      ...
      jet_thrust_cos[nfatjets_] = cosAngle;

      if    (cosAngle < -0.80) negSuperJet.push_back(candJet);   //use threshold of 0.85
      else if(cosAngle > 0.80) posSuperJet.push_back(candJet);
      else{ miscJets.push_back(candJet); }
      //miscJets.push_back(candJet);
      nfatjets_++;
   }
   ...
   for(auto iJet = fatJets->begin(); iJet != fatJets->end(); iJet++)         //get vector of all sorted superjet particles
   {
      ...
      {  
         ...
         const pat::PackedCandidate* candJetbegin = (pat::PackedCandidate*) iJ;
         double puppiweight = candJetbegin->puppiWeight();
         if     (SJMatch==1)superJetOne.push_back(fastjet::PseudoJet(puppiweight*iJ->px(),puppiweight*iJ->py(),puppiweight*iJ->pz(),puppiweight*iJ->energy()));
         else if(SJMatch==2)superJetTwo.push_back(fastjet::PseudoJet(puppiweight*iJ->px(),puppiweight*iJ->py(),puppiweight*iJ->pz(),puppiweight*iJ->energy()));
      }
   }
...
DEFINE_FWK_MODULE(clusteringAnalyzer_TprimeToHT);
