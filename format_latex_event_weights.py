...
if __name__=="__main__":
	...
	for year in years:
		for iii,plot_type in enumerate(plot_types):
				...
				output_file.write(r"\caption{ " + "The nominal %s systematic %s for the combined QCD (a), TTbar (b), single top (c), and SuuToChiChi signal (d) samples for %s }"%(plot_type.split("_")[1], syst_type[iii],year_converter(year) )+ "\n")
