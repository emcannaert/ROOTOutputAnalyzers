...
if __name__=="__main__":
	...
	for year in years:
		for plot_type in plot_types:
				...
				output_file.write(r"\caption{The true " + "%s jet b-tagging Efficiency maps used in the calculation of the %s b-tagging event weight corrections for each input dataset. Datasets of like-background are combined for larger statistics, and the signal datasets are also combined. The b jets used are selected as described"%(plot_type,year_converter(year)) + r" in \ref{section:jetID} and tagged using the DeepFlavour/DeepJet algorithm with the tight working point.}"+ "\n")
